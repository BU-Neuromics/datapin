...
- **A published documentation site, generated by datapin itself** (#54).
  <https://bu-neuromics.github.io/datapin/> is built by `datapin site publish`
  from this repository's own `.datapin/datapin.toml` — the site generator had
  never been run on a real site. It carries six task-shaped guides (first DOI,
  publishing a new version, cluster→laptop workspaces, leaving OSF, institutional
  InvenioRDM/Dataverse, scripting and CI) and a troubleshooting reference that
  covers every error datapin raises on purpose. The README keeps the command
  reference and its Documentation section is now split into "for users" and
  "for contributors" instead of pointing newcomers at the architecture plan.
...
- **`site publish` no longer reports success when GitHub Pages is serving
  something else** (#54). Enabling Pages returns 409 when it is already
  configured, and that was treated as "enabled" — so on a repository whose
  Pages source was, say, `main` at the root, datapin force-pushed the site to
  `gh-pages`, said "✓ site published", and the site never changed. The 409 path
  now reads the existing configuration back and warns loudly, naming the branch
  actually being served and where to change it. `--output=json` gained
  `pages_source_branch`, `pages_serving_site`, and `pages_url`, and a verified
  publish now prints the site URL. The read-back is best-effort: a token without
  `pages:read` reports the source as unverified rather than failing a publish
  whose bytes are already pushed.
- **The site nav labels pages by title, not by slug** (#54). A nav built from
  slugs read `first-doi` where the page is called "Your first DOI"; the
  frontmatter title now wins, with the slug as fallback for a page that declares
  none. The nav also wraps instead of overflowing horizontally once a site has
